...
const navigate=useNavigate()
    ...
    function handleRemoveFromCart(itemId) { // Change parameter name to itemId
        ...
        .then(data => {
            ...
            toast.info('Successfully removed item from cart', { position: "bottom-left" });
        })
        .catch(error => {
            ...
            {!cartItems.length ? (
                <div className="text-center mt-8">
                    ...
                    <div className="flex justify-center items-center">
                      <img src="https://cdn-icons-png.flaticon.com/512/3081/3081840.png" width="250px" className="text-center"/> 
                    </div>
                ...
                <div className="cartProducts items-center mx-auto text-center">
                    ...
                    {cartItems.map(item => (
                        <div className="flex items-center justify-between border-b py-2" key={item.id}>
                            ...
                            <div className='flex'>
                                <button className='border-2 rounded-md border-gray-300 px-3 py-3 text-2xl'>-</button>
                                <div className='border-2 border-gray-300 px-3 py-3 text-2xl'>{item.total}</div>
                                <button className='border-2 rounded-md border-gray-300 px-3 py-3 text-2xl'>+</button>
                            </div>
                            <div>${item.price ? item.price : "no price"}</div>
                            <div>${item.price ? item.price : "no price"}</div>
                        </div>
                    ))}
                    <div className="flex justify-between font-bold mb-2">
                        <div className="text-2xl">Subtotal</div>
                        <div>${cartItems.reduce((a, b) => a + b.total, 0)}</div>
                    </div>
                    <button onClick={()=>navigate('/products')} className="px-8 py-3 mt-5 bg-green-600 text-white font-bold">Continue Shopping</button>
                    <button onClick={handleCheckOut} className="px-8 py-3 m-5 bg-black text-white font-bold">CheckOut</button>
                </div>
